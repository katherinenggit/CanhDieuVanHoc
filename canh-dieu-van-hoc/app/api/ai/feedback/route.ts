...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `Bạn là giáo viên Ngữ Văn lớp 12 Việt Nam. Học sinh sai các câu sau:
${wrongQuestionsData.map((q: any, i: number) => 
  `- Câu ${i + 1}: ${q.content}. (Đáp án đúng: ${q.correctAnswer}, Học sinh chọn: ${q.userAnswer})`
).join('\n')}

Dựa trên các lỗi trên, hãy viết 1 câu nhận xét (dưới 30 từ) chỉ ra lỗ hổng kiến thức và khích lệ học sinh. Không dùng ký tự đặc biệt.`
