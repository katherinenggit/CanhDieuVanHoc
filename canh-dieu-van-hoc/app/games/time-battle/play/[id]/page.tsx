...
export default function TimeBattlePlayPage({ params }: { params: { id: string } }) {
  ...
  if (loading || sessionStatus === 'waiting') return (
    <div className="min-h-screen bg-[#85A947]-500 flex flex-col items-center justify-center  p-4">
  ...
  if (gameFinished) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#C5E1A5] to-[#FFB7C5] flex items-center justify-center p-4">
          ...
          <CardContent className="pt-10 text-center">
            ...
            <div className="space-y-3 mb-8">
              {playerScores.map((p, i) => (
                <div key={p.player_id} className={`flex items-center justify-between p-4 rounded-xl ${p.player_id === user?.id ? 'bg-[#85A947]-100 border-2 border-[#85A947]-400' : 'bg-gray-50'}`}>
                  ...
                  <span className="font-black text-[#85A947]-600 text-xl">{p.score}đ</span>
                </div>
              ))}
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Button onClick={() => router.push('/')} variant="outline" className="h-12 font-bold"><Home className="mr-2" /> Trang chủ</Button>
              <Button onClick={() => router.push('/games/time-battle')} className="h-12 font-bold bg-[#85A947] hover:bg-[#85A947]"><RotateCcw className="mr-2" /> Chơi lại</Button>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFB7C5] to-[#C5E1A5] p-4">
      <Button 
        variant="ghost" 
        size="icon" 
        className="fixed top-4 right-4 hover:bg-input/50 z-50"
      ...
      <div className="container mx-auto max-w-6xl py-8">
        <div className="flex justify-between items-end  mb-8">
          <div className="space-y-1">
            <p className="text-xs text-white uppercase font-bold opacity-70">Thời gian</p>
            <div className={`flex items-center gap-3 px-4 py-2 rounded-full border-2 ${timeRemaining <= 10 ? 'bg-red-500 border-white animate-pulse' : 'bg-black/20 border-white/20'}`}>
              ...
              <span className="text-2xl text-white font-mono font-bold">{formatTime(timeRemaining)}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 p-8 border-none shadow-2xl rounded-3xl">
                  ...
                  className={`h-auto min-h-[70px] py-4 text-lg justify-start px-6 text-left border-2 transition-all ${
                    hasAnswered && opt === questions[currentIndex]?.answer_data?.correct 
                      ...
                      : hasAnswered && selectedAnswer === opt 
                        ...
                        : selectedAnswer === opt ? 'border-[#85A947]-500 bg-[#85A947]-50' : 'hover:border-[#85A947]-200'
                  }`}
                  onClick={() => handleSelectAnswer(opt)}
                  disabled={hasAnswered}
                >
                  <span className={`mr-4 w-8 h-8 rounded-full flex items-center justify-center font-black ${selectedAnswer === opt ? 'bg-white text-[#85A947]-600' : 'bg-gray-100 text-gray-400'}`}>
            ...
            {!hasAnswered && (
              <Button 
                className="w-full mt-10 h-14 bg-[#85A947] hover:bg-[#6A8839] text-xl font-black rounded-2xl shadow-lg shadow-[#85A947]-200" 
          ...
          <Card className="p-6 border-none shadow-xl rounded-3xl h-fit">
            <h3 className="font-black text-xl mb-6 flex items-center gap-2 text-[#85A947]-800">
              <Zap className="h-6 w-6 text-[#85A947]-500 fill-[#85A947]-500"/> BXH TRỰC TIẾP
            </h3>
            <div className="space-y-3">
              {playerScores.map((p, i) => (
                <div key={p.player_id} className={`flex items-center gap-3 p-3 rounded-2xl transition-all ${p.player_id === user?.id ? 'bg-[#85A947]-50 ring-1 ring-[#85A947]-200 shadow-sm' : ''}`}>
                  <span className={`font-black w-6 text-center ${i === 0 ? 'text-yellow-500' : 'text-gray-400'}`}>{i+1}</span>
                  <Avatar className="h-10 w-10 border-2 border-white shadow-sm">
                    <AvatarImage src={p.avatar_url || ''} />
                    <AvatarFallback className="bg-[#FFB7C5] text-[#85A947]-700">{p.display_name[0]}</AvatarFallback>
                  </Avatar>
                  <span className={`flex-1 truncate font-bold ${p.player_id === user?.id ? 'text-[#85A947]-900' : 'text-gray-700'}`}>{p.display_name}</span>
                  <span className="font-black text-[#85A947]-600">{p.score}đ</span>
                </div>
              ))}
            </div>
          </Card>
